...
# ------------------------------------------------------------------- #
    ...
    def SetMessage(self, message):
        ...
        qfp = QuickFixParser.QuickFixParser(string=message)
#        tags = qfp.tags
#        for tag in tags:
#            if tag != "":
#                self.AppendItem(message_root, tag)
        ...
        for key in Dict.keys():
            print key
            if type(Dict[key]) == type(tuple()):
                newRoot = self.AppendItem( message_root, key + "=" + str(Dict[key][0]) )
                for child_key in Dict[key][1].keys():
                    childRoot = self.AppendItem( newRoot, child_key )
                    for child_child_key in Dict[key][1][child_key]:
                        self.AppendItem(childRoot, child_child_key + "=" + str(Dict[key][1][child_key][child_child_key]))
            else: self.AppendItem( message_root, qfp.CombinePair(key) )
        self.ExpandAll()
    ...
    def GetMessage(self, root=None):
        ...
        try:
            ...
            first_text = self.GetItemText(first_tag)
            if 'Message ' not in first_text:
                message = first_text + "\x01"
            if self.ItemHasChildren(first_tag):
                message = message + self.GetMessage(first_tag)
            next_tag, cookie = self.GetNextChild(message_root, cookie)
            while next_tag.IsOk():
                next_text = self.GetItemText(next_tag)
                if 'Message ' not in next_text:
                    message = message + next_text + "\x01"
