...
class MyFrame(wx.Frame):
	def __init__(self, *args, **kwds):
		...
		self.message_tree = wx.TreeCtrl(self.window_1_pane_2, -1, style=wx.TR_HAS_BUTTONS|wx.TR_NO_LINES|wx.TR_DEFAULT_STYLE|wx.SUNKEN_BORDER|wx.TR_EDIT_LABELS|wx.TR_MULTIPLE)
...
#		print "Not implemented: create message"
		...
		while next_tag.IsOk():
			...
			next_tag,cookie = self.message_tree.GetNextChild(message_root, cookie)
		message = self.message_trim_SOH(message)
		self.market_data.SetValue(message)
		
	def message_trim_SOH(self, message):
	...
	def doCopyChild(self, event):
		...
		selected = self.message_tree.GetSelections()
		if len(selected) > 1: print "Only supporting single copy at this point..."
		pyperclip.copy( str(self.message_tree.GetItemText(selected[0])) )
